...
console.log(`   Primeiros 10: ${slots30min.slice(0, 10).map(s => s.value).join(', ')}`);
...
console.log('   Sistema original (availability.tsx): 07:00, 08:00, 09:00, ..., 20:00');
console.log(`   Nova solução (60min): ${slots60min.map(s => s.value).join(', ')}`);
