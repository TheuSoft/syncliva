...
export default function DoctorReportsPage() {
  ...
  const gerarPDF = () => {
    ...
    doc.text('Relatório de Consultas - Médico', pageWidth / 2, 20, { align: 'center' });
    
    // Período
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    const periodo = tipoRelatorio === 'anual' ? `Ano: ${ano}` : `Período: ${mes}/${ano}`;
    ...
    doc.text(`Faturamento Total: ${formatCurrencyInCents(estatisticas.totalValor)}`, 14, 70);
    ...
    const tableData = agendamentos.map(ag => [
      ...
      ag.patient?.name || 'N/A',
      ag.status === 'confirmed' ? 'Confirmado' : ag.status === 'pending' ? 'Pendente' : 'Cancelado',
    ...
    // Salvar o PDF
    const nomeArquivo = tipoRelatorio === 'anual' 
    ...
    <PageContainer>
      <PageHeader>
        <PageHeaderContent>
          ...
          <PageDescription>
            Visualize e analise suas consultas realizadas, faturamento e estatísticas
          </PageDescription>
        </PageHeaderContent>
        <PageActions>
        ...
        {agendamentos.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  {tipoRelatorio === 'anual' ? `no ano ${ano}` : `em ${mes}/${ano}`}
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrencyInCents(estatisticas.totalValor)}</div>
                <p className="text-xs text-muted-foreground">
                  {tipoRelatorio === 'anual' ? `no ano ${ano}` : `em ${mes}/${ano}`}
                </p>
              </CardContent>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Consultas Confirmadas</CardTitle>
                <CalendarIcon className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{estatisticas.porStatus.confirmed?.count || 0}</div>
                <p className="text-xs text-muted-foreground">
                  {formatCurrencyInCents(estatisticas.porStatus.confirmed?.valor || 0)}
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{estatisticas.porStatus.pending?.count || 0}</div>
                <p className="text-xs text-muted-foreground">
                  {formatCurrencyInCents(estatisticas.porStatus.pending?.valor || 0)}
                </p>
              </CardContent>
            </Card>
          </div>
                    ...
                    .map(([mesAno, consultas]) => (
                      <div key={mesAno}>
                        <h3 className="text-lg font-semibold mb-3">
                          {mesAno} ({consultas.length} consultas - {formatCurrencyInCents(consultas.reduce((sum, c) => sum + c.appointmentPriceInCents, 0))})
                        </h3>
                        <Table>
                          <TableHeader>
                            <TableRow>
                          ...
                          <TableBody>
                            {consultas.map((agendamento) => (
                              <TableRow key={agendamento.id}>
                                ...
                                <TableCell>{agendamento.patient?.name || 'N/A'}</TableCell>
                                <TableCell>
                                  <Badge variant={
                                    agendamento.status === 'confirmed' ? 'default' :
                                    agendamento.status === 'pending' ? 'secondary' : 'destructive'
                                  }>
                                    {agendamento.status === 'confirmed' ? 'Confirmado' :
                                     agendamento.status === 'pending' ? 'Pendente' : 'Cancelado'}
                                  </Badge>
                                </TableCell>
                                <TableCell>{formatCurrencyInCents(agendamento.appointmentPriceInCents)}</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
